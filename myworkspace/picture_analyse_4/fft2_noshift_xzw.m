...
imshow(img_show);%显示原图
title('original image');
img_fft = fft2(img);%傅里叶变换
...
while(b)
    ...
    real = 0.5*(1+cos(2*pi*x*x_row/img_size+2*pi*y*y_row/img_size));
    imag = 0.5*(1+sin(2*pi*x*x_row/img_size+2*pi*y*y_row/img_size));
